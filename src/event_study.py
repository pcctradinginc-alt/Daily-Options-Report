...
VALID_GROUPS = {"base", "sector", "sentpx", "ev_bucket", "ivrv_bucket", "data_quality"}
...
def fetch_rows(selected_only: bool = False):
    ...
    rows = con.execute(
        ...
        SELECT s.ticker, s.direction, s.signal_strength, s.score, s.score_reason,
               ...
               s.iv_to_rv, s.option_iv, s.no_trade_reason,
...
def main():
    ...
    parser.add_argument("--group", default="base", choices=sorted(VALID_GROUPS),
                        help="Gruppierung: base, sector, sentpx, ev_bucket, ivrv_bucket, data_quality")
